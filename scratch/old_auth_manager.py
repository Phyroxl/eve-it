...
class AuthManager(QObject):
    ...
    def get_token(self) -> str | None:
        """Retorna un access token v├ílido. Alias de get_valid_access_token()."""
        return self.get_valid_access_token()

    def get_valid_access_token(self) -> str | None:
        """
        Retorna un access token v├ílido.
        - Si el token actual expira en m├ís de 120s, lo devuelve directamente.
        - Si est├í cerca de expirar o ya expir├│, intenta refresh con el refresh_token.
    ...
    def _do_refresh(self) -> bool:
        ...
        Limpia la sesi├│n autom├íticamente si CCP devuelve invalid_grant.
        """
        import requests
        import base64
        try:
            ...
            # Error handling ÔÇö do not log raw response body (may contain tokens)
            try:
                error_body = res.json()
    ...
    def _clear_session_data(self):
        """Limpia estado de autenticaci├│n en memoria y en disco. No adquiere el lock."""
        self.current_token = None
        self.refresh_token = None
        self.expiry = 0.0
        self.char_id = None
        ...
        def run_server():
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                try:
                            ...
                            if code:
                                try:
                                    ...
                                    if client_secret:
                                        ...
                                        logger.info(
                                            f"[AUTH] Token exchange (confidential) ÔåÆ HTTP {res.status_code}"
                                        )
                                    else:
                                        res = None
                                    ...
                                    if res is None or res.status_code == 401:
                                        if res is not None:
                                            logger.warning(
                                                "[AUTH] 401 with Basic auth ÔåÆ retrying as public client"
                                            )
                                        ...
                                        logger.info(
                                            f"[AUTH] Token exchange (public/native) ÔåÆ HTTP {res.status_code}"
                                        )
                                    ...
                                    if res.status_code == 200:
                                        ...
                                        try:
                                            ...
                                            logger.warning(
                                                f"[AUTH] JWT decode failed: {jwt_err} ÔÇö using /oauth/verify"
                                            )
                                            ...
                                            if verify_res.status_code == 200:
                                                ...
                                                logger.error(
                                                    f"[AUTH] /oauth/verify ÔåÆ {verify_res.status_code}"
                                                )
                    ...
                    self.auth_error = "Timeout ÔÇö user did not complete login in 60s."
    ...
    def save_session(self):
        """Guarda la sesi├│n actual en config/esi_session.json (incluye access_token y expires_at)."""
    ...
    def try_restore_session(self) -> str:
        """
        Intenta restaurar la sesi├│n desde el archivo guardado.
        Retorna: 'ok', 'no_session', 'expired', 'new_scopes_required'

        Si el access_token guardado sigue siendo v├ílido, se usa directamente sin
        hacer ninguna llamada de red. Si est├í caducado, se refresca usando el
        ...
        """
        emit_args = None  # (char_name, tokens) ÔÇö se emite fuera del lock

        with self._lock:
            # Si ya hay un token v├ílido en memoria, no repetir el restore
            if self.current_token and time.time() < self.expiry - _TOKEN_REFRESH_MARGIN:
            ...
            # Leer y parsear el archivo de sesi├│n
            try:
            ...
            # Si el access_token guardado a├║n es v├ílido, usarlo sin llamada de red
            ...
            if saved_token and time.time() < saved_expires_at - _TOKEN_REFRESH_MARGIN:
                ...
                # Token caducado o cerca de caducar ÔåÆ refrescar
                ...
                if self._do_refresh():
                    ...
                    # _do_refresh limpia la sesi├│n si fue invalid_grant
                    return "expired"
    ...
    def logout(self):
        """Cierra sesi├│n: borra el archivo persistente y limpia el estado en memoria."""
        self._clear_session_data()
    ...
    def get_auth_status(self) -> dict:
        """Retorna el estado actual de autenticaci├│n. No incluye valores de tokens."""
