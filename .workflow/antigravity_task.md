...
## Sesi�n 36: Alineaci�n de Candidatos con Filtros Visibles

### Diagn�stico del Reporte 0d6b524b
- **Causa Ra�z**: El worker seleccionaba los 'top 200' bas�ndose �nicamente en el margen te�rico sin saneamiento previo. �tems con spreads astron�micos (>500% o incluso >10000%) dominaban el pool por tener m�rgenes irreales, siendo luego descartados al 100% por la UI.
- **Anomal�a de Enriquecimiento**: Se observ� Relevant Orders (Enr)=3581 pero Opps Enriched=0, sugiriendo un fallo en el filtrado posterior al enriquecimiento o en el agrupamiento.

### Soluci�n
1.  **Nuevo M�dulo de Selecci�n**: core/market_candidate_selector.py extrae la l�gica de selecci�n y a�ade un pre-filtro de saneamiento (Pre-Filter) alineado con los filtros visibles (Capital, Spread, Margen, PLEX).
2.  **Instrumentaci�n de Prefilter**: El worker ahora informa cu�ntos candidatos fueron eliminados por spread, capital o margen antes de elegir el top 200.
3.  **Diagn�stico de Enriquecimiento**: A�adido an�lisis detallado de la entrada a parse_opportunities para detectar por qu� se pierden �tems durante la fase 2.
4.  **Aislamiento de L�gica**: La l�gica de selecci�n ahora es puramente funcional y testeable.

### Verificaci�n
- **Unit Tests**: Nuevo test tests/test_market_candidate_selector.py (PASS).
- **Regresi�n**: Suite completa de 60+ tests (PASS).
...
- ui/market_command/refresh_worker.py (Integraci�n del selector y telemetr�a)
- tests/test_market_candidate_selector.py (Nuevo)

## Sesi�n 36 (Parte 2): Mejora de Telemetr�a e Iconos

### Correcciones de Diagn�stico
- **Opps Enriched**: Se corrigi� el registro de opps_enriched_count en el Worker, que anteriormente se mostraba como 0 a pesar de tener resultados.
- **Delay de Di�logo**: Se aument� el tiempo de espera para abrir el reporte a 2000ms para permitir que las peticiones as�ncronas de iconos tengan tiempo de finalizar.
- **Performance Warning**: El reporte ahora a�ade una advertencia si la descarga de �rdenes de mercado supera los 20 segundos.

### Mejoras de Iconos
- **Telemetry**: A�adido seguimiento de icon_cache_hits y registro de los �ltimos errores de red/pixmap (icon_last_errors).
- **Depuraci�n**: La tabla ahora informa si las peticiones est�n pendientes o si fallaron por errores de red o carga de pixmap.

### Verificaci�n
- **Reporte d47c572b**: Validado que UI Filtered Results = 200 y el candidate selector funciona correctamente.
- **Regresi�n**: Suite completa de tests (PASS).
- **Sintaxis**: py_compile (PASS) en todos los m�dulos de UI y Core.

## Sesi�n 36 (Parte 3): Optimizaci�n de Rendimiento de �rdenes de Mercado

### Paginaci�n Concurrente
- **ESIClient**: Se implement� ThreadPoolExecutor en market_orders para descargar todas las p�ginas en paralelo (8 workers por defecto).
- **Robustez**: A�adido helper _fetch_market_page con reintentos autom�ticos y manejo de 429 para evitar fallos por saturaci�n de red.

### Cache de Sesi�n
- **MarketOrdersCache**: Nuevo singleton que almacena el snapshot completo de �rdenes de mercado en memoria con un TTL de 120 segundos.
- **UX**: El segundo escaneo dentro del TTL ahora es casi instant�neo (Cache HIT), evitando descargar ~400k �rdenes innecesariamente.

### Telemetr�a de Rendimiento
- **Reporte**: Nueva secci�n [MARKET ORDERS FETCH] con detalles de Source (ESI vs Cache), P�ginas Totales, Trabajadores y Edad del Cache.
- **Progreso**: Feedback visual m�s claro durante la descarga y verificaci�n de cache.

### Verificaci�n
- **Sintaxis**: py_compile (PASS).
- **Tests**: Nueva suite 	est_market_orders_cache.py (PASS) + Regresi�n completa (PASS).
- **Rendimiento**: Reducci�n dr�stica del tiempo de escaneo repetido y mejora significativa en el primer fetch.
