...
def analyze_character_orders(esi_orders: List[Dict[str, Any]], market_orders: List[Dict[str, Any]], item_names: Dict[int, str] = None, config: FilterConfig = None, char_id: int = 0):
    from .market_models import OpenOrder, OpenOrderAnalysis
    if config is None: config = FilterConfig()
    if item_names is None: item_names = {}
    
    # Obtener tasas reales por personaje
    tax_info = TaxService.instance().get_taxes(char_id)
    b_fee = tax_info.broker_fee_pct / 100.0
    ...
    for eo in esi_orders:
        ...
        if is_buy:
            ...
            difference = price - bs
            competitive = difference <= 0
            # Ventas: Profit REAL (basado en Mi Promedio / WAC)
            if avg_cost > 0:
                base_cost = avg_cost
        ...
        o = OpenOrder(order_id=eo['order_id'], type_id=t_id, item_name=item_names.get(t_id, f"Type {t_id}"), is_buy_order=is_buy, price=price, volume_total=eo.get('volume_total', 0), volume_remain=vol_remain, issued=eo.get('issued', ''), location_id=eo.get('location_id', 0), range=eo.get('range', ''), analysis=analysis)
        parsed_orders.append(o)
    return parsed_orders

def analyze_inventory(assets: List[Dict[str, Any]], market_orders: List[Dict[str, Any]], item_names: Dict[int, str] = None, config: FilterConfig = None, char_id: int = 0):
    if config is None: config = FilterConfig()
    if item_names is None: item_names = {}
    tax_info = TaxService.instance().get_taxes(char_id)
    b_fee = tax_info.broker_fee_pct / 100.0
    ...
    for t_id, info in grouped_assets.items():
        ...
        results.append(InventoryItem(type_id=t_id, item_name=item_names.get(t_id, f"Type {t_id}"), quantity=qty, location_id=info['location'], analysis=analysis))
    return results
