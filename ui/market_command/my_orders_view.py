import logging # VERSION: 1.1.15-FIXWAC (Robust WAC & Scope Detection)
import time
import threading
...
class SyncWorker(QThread):
    ...
    def run(self):
        try:
            ...
            self.status_update.emit("ANALIZANDO...", 95)
...
class InventoryWorker(QThread):
    ...
    def run(self):
        try:
            ...
            curr_loc_id, loc_name = None, "TODO EL INVENTARIO (FALLBACK)"
            
            if loc_res == "missing_scope": self.location_info.emit("missing_scope")
            elif loc_res:
            ...
            if assets == "missing_scope": self.error.emit("missing_scope"); return
            if not assets: self.finished_data.emit([]); return
            
            filtered = [a for a in assets if a.get('location_id') == curr_loc_id] if curr_loc_id else [a for a in assets if 'slot' not in a.get('location_flag', '').lower()]
            if not filtered: self.finished_data.emit([]); return

            self.status_update.emit("BUSCANDO PRECIOS JITA...", 70)
            ...
            analyzed = analyze_inventory(filtered, all_market_orders, item_names, load_market_filters(), char_id=self.char_id, token=self.token)
...
class InventoryAnalysisDialog(QDialog):
    def __init__(self, items, loc_name, image_loader, parent=None):
        ...
        self.items, self.loc_name, self.image_loader = items, loc_name, image_loader
        self.setWindowTitle("INVENTARIO - VALOR DE ACTIVOS")
        self.setMinimumSize(1150, 750); self.setStyleSheet("background-color: #000000;")
        self.setup_ui()

    def setup_ui(self):
        layout = QVBoxLayout(self); layout.setContentsMargins(20, 20, 20, 20); layout.setSpacing(15)
        header = QFrame(); header.setFixedHeight(85); header.setStyleSheet("background-color: #0f172a; border-radius: 8px; border: 1px solid #1e293b;")
        hl = QHBoxLayout(header); title_v = QVBoxLayout()
        title = QLabel("INVENTARIO LOCAL"); title.setStyleSheet("color: #f1f5f9; font-size: 18px; font-weight: 900; border:none;")
        loc_lbl = QLabel(f"UBICACIÓN: {self.loc_name.upper()}"); loc_lbl.setStyleSheet(f"color: {'#3b82f6' if 'FALLBACK' not in self.loc_name else '#f59e0b'}; font-size: 10px; font-weight: 800; border:none;")
        title_v.addWidget(title); title_v.addWidget(loc_lbl); hl.addLayout(title_v); hl.addStretch()
        
        total_val = sum(item.analysis.est_total_value for item in self.items)
        val_v = QVBoxLayout(); val_lbl = QLabel(format_isk(total_val)); val_lbl.setStyleSheet("color: #10b981; font-size: 22px; font-weight: 900; border:none;")
        val_sub = QLabel("VALOR ESTIMADO NETO"); val_sub.setStyleSheet("color: #64748b; font-size: 9px; font-weight: 800; border:none;"); val_sub.setAlignment(Qt.AlignRight)
        val_v.addWidget(val_lbl); val_v.addWidget(val_sub); hl.addLayout(val_v); layout.addWidget(header)

        self.table = QTableWidget(len(self.items), 9); self.table.setHorizontalHeaderLabels(["", "ÍTEM", "CANTIDAD", "MI PROMEDIO", "P. UNIT NETO", "PROFIT DE VENTA", "VALOR %", "RECOMENDACIÓN", "MOTIVO"])
        self.table.setEditTriggers(QAbstractItemView.NoEditTriggers); self.table.setShowGrid(False)
        self.table.setStyleSheet("QTableWidget { background: #000000; color: #f1f5f9; border: none; font-size: 11px; } QHeaderView::section { background: #000000; color: #64748b; font-weight: 900; border: none; border-bottom: 1px solid #1e293b; padding: 10px; } QTableWidget::item { border-bottom: 1px solid #0f172a; padding: 8px; } QTableWidget::item:selected { background: #0f172a; color: white; }")
        self.table.horizontalHeader().setStretchLastSection(True); self.table.verticalHeader().setVisible(False); self.table.setIconSize(QSize(32, 32))
        ...
        for row, item in enumerate(sorted(self.items, key=lambda x: getattr(x, "_net_profit_total", 0.0), reverse=True)):
            a = item.analysis; avg_buy = getattr(item, "_avg_buy", 0.0); profit_t = getattr(item, "_net_profit_total", 0.0)
            i_icon = QTableWidgetItem(); i_icon.setData(Qt.UserRole, item.type_id)
            ...
            i_qty = NumericTableWidgetItem(f"{item.quantity:,}", item.quantity); i_qty.setTextAlignment(Qt.AlignCenter)
            
            # MI PROMEDIO: Si es 0, mostrar Sin registros
            i_avg = NumericTableWidgetItem(format_isk(avg_buy) if avg_buy > 0 else "Sin registros", avg_buy)
            i_avg.setTextAlignment(Qt.AlignRight | Qt.AlignVCenter); i_avg.setForeground(QColor("#94a3b8") if avg_buy > 0 else QColor("#334155"))
            
            i_price = NumericTableWidgetItem(format_isk(a.est_net_sell_unit), a.est_net_sell_unit); i_price.setTextAlignment(Qt.AlignRight | Qt.AlignVCenter)
            
            # PROFIT: Solo mostrar si tenemos coste (WAC)
            i_profit = NumericTableWidgetItem(format_isk(profit_t) if avg_buy > 0 else "Sin registros", profit_t if avg_buy > 0 else -1e18)
            i_profit.setTextAlignment(Qt.AlignRight | Qt.AlignVCenter)
            if avg_buy > 0: i_profit.setForeground(QColor("#10b981" if profit_t >= 0 else "#ef4444"))
            else: i_profit.setForeground(QColor("#334155"))
            
            pct = (a.est_total_value / total_val * 100) if total_val > 0 else 0
            i_pct = NumericTableWidgetItem(f"{pct:.1f}%", pct); i_pct.setTextAlignment(Qt.AlignCenter)
            
            i_rec = SemanticTableWidgetItem(a.recommendation.upper()); i_rec.setTextAlignment(Qt.AlignCenter)
        ...
        self.table.setSortingEnabled(True); self.table.itemDoubleClicked.connect(self.on_double_click); layout.addWidget(self.table)
        footer = QLabel("* Si falta el registro de coste (WAC), reautoriza el personaje para permitir lectura de wallet."); footer.setStyleSheet("color: #3b82f6; font-size: 9px; font-weight: 800;"); layout.addWidget(footer)

    def on_double_click(self, item):
        tid = self.table.item(item.row(), 0).data(Qt.UserRole); name = self.table.item(item.row(), 1).text()
...
class MarketMyOrdersView(QWidget):
    def __init__(self, parent=None):
        ...
        self.setup_ui(); self._load_ui_state()
    ...
    def setup_ui(self):
        ...
        header = QHBoxLayout(); title_v = QVBoxLayout()
        ...
        status_h.addWidget(self.lbl_spinner); status_h.addWidget(self.lbl_status); status_h.addStretch(); title_v.addWidget(title_lbl); title_v.addLayout(status_h)
        self.progress_bar = QProgressBar(); self.progress_bar.setFixedHeight(4); self.progress_bar.setTextVisible(False)
        self.progress_bar.setStyleSheet("QProgressBar { background: #1e293b; border: none; } QProgressBar::chunk { background: #3b82f6; }"); self.progress_bar.hide(); title_v.addWidget(self.progress_bar)
        ...
        self.btn_repopulate.clicked.connect(lambda: self.do_sync(is_update=True)); self.btn_refresh.clicked.connect(lambda: self.do_sync(is_update=False)); self.btn_inventory.clicked.connect(self.do_inventory_analysis)
        header.addLayout(title_v); header.addStretch(); header.addWidget(self.btn_inventory); header.addSpacing(10); header.addWidget(self.btn_repopulate); header.addWidget(self.btn_refresh); self.main_layout.addLayout(header)

        self.lbl_sell_count = QLabel("ÓRDENES DE VENTA (0)"); self.lbl_sell_count.setStyleSheet("color:#ef4444; font-weight:900; font-size:10px;"); self.main_layout.addWidget(self.lbl_sell_count)
        self.table_sell = self.create_table(is_buy=False); self.main_layout.addWidget(self.table_sell, 1); self.setup_taxes_bar()
        self.lbl_buy_count = QLabel("ÓRDENES DE COMPRA (0)"); self.lbl_buy_count.setStyleSheet("color:#3b82f6; font-weight:900; font-size:10px;"); self.main_layout.addWidget(self.lbl_buy_count)
        self.table_buy = self.create_table(is_buy=True); self.main_layout.addWidget(self.table_buy, 1)

        self.detail_panel = QFrame(); self.detail_panel.setFixedHeight(130); self.detail_panel.setStyleSheet("background-color: #000000; border: 1px solid #1e293b; border-radius: 4px;"); self.setup_detail_layout(); self.main_layout.addWidget(self.detail_panel)
    ...
    def setup_taxes_bar(self):
        ...
        l = QHBoxLayout(self.taxes_bar); l.setContentsMargins(15, 0, 15, 0)
        ...
        l.addWidget(self.lbl_sales_tax); l.addSpacing(20); l.addWidget(self.lbl_broker_fee); l.addStretch(); l.addWidget(self.lbl_tax_source); self.main_layout.addWidget(self.taxes_bar)

    def create_table(self, is_buy=False):
        t = QTableWidget(0, 12); t.setHorizontalHeaderLabels(["", "ÍTEM", "TIPO", "MI PRECIO", "MI PROMEDIO", "MEJOR VENTA" if is_buy else "MEJOR COMPRA", "TOTAL", "RESTO", "SPREAD", "MARGEN", "PROFIT", "ESTADO"])
        ...
        t.itemSelectionChanged.connect(self.on_selection_changed); t.itemDoubleClicked.connect(lambda i: self.on_double_click(i, t)); t.setSortingEnabled(True); return t

    def setup_detail_layout(self):
        dl = QHBoxLayout(self.detail_panel); dl.setContentsMargins(15, 12, 15, 12)
        self.lbl_det_icon = QLabel(); self.lbl_det_icon.setFixedSize(64, 64); dl.addWidget(self.lbl_det_icon)
    ...
    def _start_sync_ui(self, msg): self.btn_refresh.setEnabled(False); self.btn_repopulate.setEnabled(False); self.btn_inventory.setEnabled(False); self.lbl_status.setText(msg); self.lbl_status.setStyleSheet("color: #3b82f6;"); self.progress_bar.setValue(0); self.progress_bar.show(); self.spinner_timer.start(100)
    def _stop_sync_ui(self, msg, color="#10b981"): self.btn_refresh.setEnabled(True); self.btn_repopulate.setEnabled(True); self.btn_inventory.setEnabled(True); self.lbl_status.setText(msg); self.lbl_status.setStyleSheet(f"color: {color};"); self.lbl_spinner.setText(""); self.spinner_timer.stop(); QTimer.singleShot(3000, lambda: self.progress_bar.hide())

    def do_sync(self, is_update=False):
        auth = AuthManager.instance(); t = auth.get_token()
        if not t: auth.login(); return
        self._start_sync_ui("SINCRONIZANDO ÓRDENES..."); self.worker = SyncWorker(auth.char_id, t); self.worker.status_update.connect(lambda m, v: (self.lbl_status.setText(m), self.progress_bar.setValue(v))); self.worker.finished_data.connect(self.on_data_ready); self.worker.error.connect(self.on_error); self.worker.start()

    def on_data_ready(self, orders): self.all_orders = orders; self.update_taxes_info(); self.populate_all(orders); self._stop_sync_ui(f"SINCRONIZACIÓN EXITOSA ({len(orders)} ÓRDENES)")
    ...
    def fill_table(self, t, data):
        ...
        for r, o in enumerate(data):
            ...
            i_name = QTableWidgetItem(o.item_name); i_type = QTableWidgetItem("BUY" if o.is_buy_order else "SELL"); i_type.setForeground(QColor("#3b82f6" if o.is_buy_order else "#ef4444"))
            i_price = NumericTableWidgetItem(format_isk(o.price), o.price); avg = cost.average_buy_price if cost else 0.0
            ...
            i_tot = NumericTableWidgetItem(str(o.volume_total), o.volume_total); i_rem = NumericTableWidgetItem(str(o.volume_remain), o.volume_remain)
            i_spr = NumericTableWidgetItem(f"{a.spread_pct:.1f}%", a.spread_pct); i_mar = NumericTableWidgetItem(f"{a.margin_pct:.1f}%" if a.margin_pct != 0 else "---", a.margin_pct)
            ...
            i_state = SemanticTableWidgetItem(a.state.upper()); s_low = a.state.lower()
            if any(x in s_low for x in ["liderando", "competitiva", "sana", "rentable"]): i_state.setForeground(QColor("#3b82f6" if o.is_buy_order else "#10b981"))
    ...
    def on_selection_changed(self):
        ...
        si = s.selectedItems()
        if not si: return
        oid = s.item(si[0].row(), 0).data(Qt.UserRole + 1); o = next((ord for ord in self.all_orders if ord.order_id == oid), None)
        if o: self.update_det(o)

    def update_det(self, o):
        self.lbl_det_item.setText(o.item_name.upper()); self.lbl_det_type.setText("COMPRA" if o.is_buy_order else "VENTA"); self.lbl_det_type.setStyleSheet("color:#3b82f6;" if o.is_buy_order else "color:#ef4444;")
        ...
        self.lbl_det_cost_msg.setText(f"Info: {a.state} | Fee: {getattr(o, '_b_fee_pct', 3.0):.2f}%")
        ...
        self.det_margin.setText(f"{a.margin_pct:.1f}%"); self.det_profit_u.setText(format_isk(a.net_profit_per_unit)); self.det_profit_t.setText(format_isk(a.net_profit_total)); self.det_state.setText(a.state.upper())

    def on_double_click(self, item, t):
        tid = t.item(item.row(), 0).data(Qt.UserRole); name = t.item(item.row(), 1).text(); ItemInteractionHelper.open_market_with_fallback(ESIClient(), AuthManager.instance().char_id, tid, name, lambda m, c: self.lbl_status.setText(m))

    def do_inventory_analysis(self):
        auth = AuthManager.instance(); t = auth.get_token()
        if not t: auth.login(); return
        self._start_sync_ui("CARGANDO INVENTARIO..."); self.inventory_status = "loading"
        self.inv_worker = InventoryWorker(auth.char_id, t); self.inv_worker.status_update.connect(lambda m, v: (self.lbl_status.setText(m), self.progress_bar.setValue(v)))
        def on_done(data):
    ...
    def update_taxes_info(self):
        ...
        self.lbl_sales_tax.setText(f"SALES TAX: {tx.sales_tax_pct:.2f}%"); self.lbl_broker_fee.setText(f"BROKER FEE: {tx.broker_fee_pct:.2f}% (BASE)"); self.lbl_tax_source.setText(f"FUENTE: {'REAL' if tx.status=='ready' else 'FALLBACK'}")
