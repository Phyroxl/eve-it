import logging # VERSION: 1.1.19-FIXSYNTAX (Resolved Compaction Errors)
import time
import threading
import json
import os
...
class TradeProfitsWorker(QThread):
    ...
    def run(self):
        try:
            ...
            for t in sorted_tx:
                tid, qty, price = t['type_id'], t['quantity'], t['unit_price']
                is_buy, loc_id = t.get('is_buy', False), t.get('location_id', 0)
                ...
                if is_buy:
                    ...
                    if curr['qty'] > 0:
                        ...
                        trades.append({
                            'date': t['date'], 'type_id': tid, 'name': item_names.get(tid, f"Item {tid}"),
                            'qty': qty, 'buy_unit': wac_unit, 'sell_unit': price,
                            'buy_total': total_cost, 'sell_total': gross_sell, 'fees': fees,
...
class TradeProfitsDialog(QDialog):
    def __init__(self, char_id, token, image_loader, parent=None):
        ...
        self.setWindowTitle("TRADE PROFITS"); self.setMinimumSize(1200, 750)
    ...
    def setup_ui(self):
        ...
        self.txt_filter = QLineEdit(); self.txt_filter.setPlaceholderText("Filtrar item..."); self.txt_filter.setFixedWidth(250)
        self.txt_filter.textChanged.connect(self.apply_filters)
        self.cmb_mode = QComboBox(); self.cmb_mode.addItems(["Todos", "Ganancias", "Pérdidas"])
        self.cmb_mode.currentIndexChanged.connect(self.apply_filters)
        fl.addWidget(QLabel("ÍTEM:")); fl.addWidget(self.txt_filter); fl.addWidget(QLabel("MODO:")); fl.addWidget(self.cmb_mode); fl.addStretch()
        ...
        self.table.setEditTriggers(QAbstractItemView.NoEditTriggers); self.table.setSortingEnabled(True)
        ...
        nav = QHBoxLayout(); self.btn_prev = QPushButton("ANTERIOR"); self.btn_next = QPushButton("SIGUIENTE"); self.lbl_page = QLabel("Página 1")
    ...
    def on_data(self, data): self.all_trades = data; self.apply_filters()
    def apply_filters(self):
        txt, mode = self.txt_filter.text().lower(), self.cmb_mode.currentIndex()
    ...
    def update_table(self):
        ...
        page_items = self.filtered_trades[start:end]; self.table.setRowCount(len(page_items))
        for r, t in enumerate(page_items):
            ...
            i_mar = NumericTableWidgetItem(f"{t['margin']:.1f}%", t['margin']); i_mar.setForeground(m_col); self.table.setItem(r, 9, i_mar)
            i_prof = NumericTableWidgetItem(format_isk(t['profit']), t['profit']); i_prof.setForeground(QColor("#10b981" if t['profit'] > 0 else "#ef4444")); self.table.setItem(r, 10, i_prof)
        self.table.setSortingEnabled(True)
        tot = (len(self.filtered_trades) + self.page_size - 1) // self.page_size
        self.lbl_page.setText(f"Página {self.current_page + 1} de {max(1, tot)}"); self.btn_prev.setEnabled(self.current_page > 0); self.btn_next.setEnabled(end < len(self.filtered_trades))
...
class MarketMyOrdersView(QWidget):
    ...
    def setup_ui(self):
        ...
        title_lbl = QLabel("MIS PEDIDOS"); title_lbl.setStyleSheet("color:#f1f5f9; font-size:18px; font-weight:900;")
        status_h = QHBoxLayout(); self.lbl_spinner = QLabel(""); self.lbl_spinner.setFixedWidth(15); self.lbl_status = QLabel("● ESPERANDO SINCRONIZACIÓN")
    ...
    def create_table(self, is_buy):
        ...
        t.verticalHeader().setVisible(False); t.setSelectionBehavior(QAbstractItemView.SelectRows); t.horizontalHeader().setSectionsMovable(True)
        t.setSortingEnabled(True); return t
    ...
    def save_layouts(self):
        cfg = {"w": [self.table_sell.columnWidth(i) for i in range(12)], "v": [self.table_sell.horizontalHeader().visualIndex(i) for i in range(12)]}
        save_ui_config("my_orders", cfg)
    def load_layouts(self):
        cfg = load_ui_config("my_orders"); w, v = cfg.get("w"), cfg.get("v")
        if w: 
    ...
    def do_sync(self):
        ...
        self.worker = SyncWorker(auth.char_id, t); self.worker.finished_data.connect(self.on_data); self.worker.start()
        self.spinner_timer.start(100); self.lbl_status.setText("SINCRONIZANDO..."); self.btn_repopulate.setEnabled(False)
    ...
    def on_data(self, data):
        ...
        self.spinner_timer.stop(); self.lbl_spinner.setText(""); self.lbl_status.setText("LISTO"); self.btn_repopulate.setEnabled(True)
