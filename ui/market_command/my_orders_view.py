import logging # VERSION: 1.1.4-PRO (WAC, Skills, Counters, Lock UI)
...
class SyncWorker(QThread):
    ...
    def run(self):
        try:
            ...
            # Actualizar tasas e impuestos según skills
            ...
            # Refrescar WAC (Weighted Average Cost)
...
class InventoryAnalysisDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        for row, item in enumerate(sorted_items):
            ...
            for c, it in enumerate([i_icon, i_name, i_qty, i_price, i_total, i_pct]): self.table.setItem(row, c, it)
...
class MarketMyOrdersView(QWidget):
    ...
    def setup_ui(self):
        ...
        header_layout.addLayout(title_v); header_layout.addStretch()
        header_layout.addWidget(self.btn_inventory); header_layout.addSpacing(10)
        header_layout.addWidget(self.btn_repopulate); header_layout.addWidget(self.btn_refresh)
        self.main_layout.addLayout(header_layout)

        def create_t():
            t = QTableWidget(0, 12)
            t.setHorizontalHeaderLabels(["", "ÍTEM", "TIPO", "MI PRECIO", "MI PROMEDIO", "MEJOR COMPRA", "TOTAL", "RESTO", "SPREAD", "MARGEN", "PROFIT", "ESTADO"])
            t.verticalHeader().setVisible(False); t.setSelectionBehavior(QAbstractItemView.SelectRows)
            ...
            t.horizontalHeader().setStretchLastSection(True); t.setIconSize(QSize(24, 24))
            t.setColumnWidth(0, 32); t.setColumnWidth(1, 180)
            return t

        self.table_sell = create_t(); self.table_buy = create_t()
        ...
        self.detail_panel = QFrame(); self.detail_panel.setFixedHeight(130)
    ...
    def setup_detail_layout(self):
        ...
        self.lbl_det_icon = QLabel(); self.lbl_det_icon.setFixedSize(64, 64); self.lbl_det_icon.setStyleSheet("background:#1e293b; border-radius:4px;")
        dl.addWidget(self.lbl_det_icon)
        info_v = QVBoxLayout()
        self.lbl_det_item = QLabel("SELECCIONA UNA ORDEN"); self.lbl_det_item.setStyleSheet("color:#f1f5f9; font-size:14px; font-weight:900;")
        self.lbl_det_type = QLabel("---"); self.lbl_det_type.setStyleSheet("color:#64748b; font-size:10px; font-weight:800;")
        self.lbl_det_cost_msg = QLabel("---"); self.lbl_det_cost_msg.setStyleSheet("color:#475569; font-size:9px; font-weight:700;")
        info_v.addWidget(self.lbl_det_item); info_v.addWidget(self.lbl_det_type); info_v.addWidget(self.lbl_det_cost_msg); info_v.addStretch()
        dl.addLayout(info_v, 2)
        self.grid = QGridLayout(); self.grid.setSpacing(10)
        def add_m(l, r, c):
            v = QVBoxLayout(); lbl = QLabel(l, styleSheet="color:#475569; font-size:9px; font-weight:900;")
            val = QLabel("---", styleSheet="color:#f1f5f9; font-size:11px; font-weight:900;")
            v.addWidget(lbl); v.addWidget(val); self.grid.addLayout(v, r, c); return val
        self.det_price = add_m("MI PRECIO", 0, 0); self.det_avg = add_m("MI PROMEDIO", 0, 1)
        self.det_best_buy = add_m("MEJOR COMPRA", 0, 2); self.det_best_sell = add_m("MEJOR VENTA", 0, 3)
        self.det_margin = add_m("MARGEN NETO", 1, 0); self.det_profit_u = add_m("PROFIT / U", 1, 1)
        self.det_profit_t = add_m("PROFIT TOTAL", 1, 2); self.det_state = add_m("ESTADO", 1, 3)
        dl.addLayout(self.grid, 5)

    def _on_header_resized(self, st, i, s):
        if self._syncing_headers: return
        self._syncing_headers = True
        target = self.table_buy if st == self.table_sell else self.table_sell
        target.setColumnWidth(i, s); self._syncing_headers = False; self._save_ui_state()

    def _on_header_moved(self, st, i, o, n):
        if self._syncing_headers: return
        self._syncing_headers = True
        target = self.table_buy if st == self.table_sell else self.table_sell
        target.horizontalHeader().moveSection(o, n); self._syncing_headers = False; self._save_ui_state()

    def _save_ui_state(self):
        h = self.table_sell.horizontalHeader()
        state = {"widths": [self.table_sell.columnWidth(i) for i in range(self.table_sell.columnCount())], "order": [h.visualIndex(i) for i in range(self.table_sell.columnCount())]}
    ...
    def _load_ui_state(self):
        ...
        self._syncing_headers = True
        try:
            widths = state.get("widths", []); order = state.get("order", [])
            for i, w in enumerate(widths):
                if i < self.table_sell.columnCount(): self.table_sell.setColumnWidth(i, w); self.table_buy.setColumnWidth(i, w)
            for li, vi in enumerate(order):
                if li < self.table_sell.columnCount():
                    self.table_sell.horizontalHeader().moveSection(self.table_sell.horizontalHeader().visualIndex(li), vi)
                    self.table_buy.horizontalHeader().moveSection(self.table_buy.horizontalHeader().visualIndex(li), vi)
        finally: self._syncing_headers = False

    def do_sync(self, is_update=False):
        auth = AuthManager.instance(); if not auth.current_token: auth.login(); return
        ...
        self.worker.finished_data.connect(self.on_data_ready); self.worker.error.connect(self.on_error); self.worker.start()

    def on_data_ready(self, orders):
        self.all_orders = orders; self.populate_all(orders)
    ...
    def _start_inventory_preload(self):
        auth = AuthManager.instance(); if not auth.current_token: return
        self.inventory_status = "loading"; self.inv_worker = InventoryWorker(auth.char_id, auth.current_token)
        self.inv_worker.finished_data.connect(self._on_inv_preloaded); self.inv_worker.error.connect(self._on_inv_error); self.inv_worker.start()

    def _on_inv_preloaded(self, data):
        self.inventory_cache = data; self.inventory_status = "ready"
    def _on_inv_error(self, err):
        self.inventory_status = "error"; self.inventory_error_msg = err
    ...
    def populate_all(self, orders):
        ...
        def fill(t, data):
            ...
            for r, o in enumerate(data):
                ...
                i_state = QTableWidgetItem(a.state); s_txt = a.state.lower()
                ...
                items = [i_ico, QTableWidgetItem(o.item_name), QTableWidgetItem("BUY" if o.is_buy_order else "SELL"), QTableWidgetItem(format_isk(o.price)), i_avg, QTableWidgetItem(format_isk(a.best_buy if o.is_buy_order else a.best_sell)), QTableWidgetItem(str(o.volume_total)), QTableWidgetItem(str(o.volume_remain)), QTableWidgetItem(f"{a.spread_pct:.1f}%"), QTableWidgetItem(f"{a.margin_pct:.1f}%" if not o.is_buy_order and cost else "---"), QTableWidgetItem(format_isk(a.net_profit_total) if not o.is_buy_order and cost else "---"), i_state]
    ...
    def on_sell_selection_changed(self): self.table_buy.clearSelection(); self._handle_sel(self.table_sell)
    def on_buy_selection_changed(self): self.table_sell.clearSelection(); self._handle_sel(self.table_buy)
    def _handle_sel(self, t):
        si = t.selectedItems(); if not si: return
    ...
    def update_det(self, o):
        ...
        if o.is_buy_order: self.lbl_det_cost_msg.setText("Profit potencial basado en Jita Sell")
        elif avg > 0: self.lbl_det_cost_msg.setText("Profit real basado en Coste Medio Ponderado (WAC)")
        else: self.lbl_det_cost_msg.setText("Sin registros de coste real para calcular beneficio")
        ...
        s_txt = a.state.lower(); self.det_state.setText(a.state.upper())
        ...
        if (not o.is_buy_order and avg > 0) or o.is_buy_order:
            ...
            for l in [self.det_margin, self.det_profit_u, self.det_profit_t]: l.setText("---"); l.setStyleSheet("color:#475569;")
    ...
    def do_inventory_analysis(self):
        if self.inventory_status == "ready" and self.inventory_cache is not None:
            if not self.inventory_cache: QMessageBox.information(self, "Inventario Vacío", "No se encontraron activos valorables."); return
            InventoryAnalysisDialog(self.inventory_cache, self.image_loader, self).exec(); return
        if self.inventory_status == "loading": QMessageBox.information(self, "Cargando", "Analizando inventario..."); return
        if self.inventory_status == "error": self.on_inventory_error(self.inventory_error_msg); self.inventory_status = "idle"; return
        self.lbl_status.setText("● CARGANDO INVENTARIO..."); self.lbl_status.setStyleSheet("color:#3b82f6;")
        auth = AuthManager.instance(); if not auth.current_token: auth.login(); return
        self.inventory_status = "loading"; self.inv_worker = InventoryWorker(auth.char_id, auth.current_token)
        def on_done(data): 
            self.inventory_cache = data; self.inventory_status = "ready"
            if not data: QMessageBox.information(self, "Inventario Vacío", "No se encontraron activos valorables.")
            else: InventoryAnalysisDialog(data, self.image_loader, self).exec()
        self.inv_worker.finished_data.connect(on_done); self.inv_worker.error.connect(self._on_inventory_error); self.inv_worker.start()

    def _on_inventory_error(self, msg): self.on_inventory_error(msg)
    ...
    def on_error(self, err):
        self.btn_refresh.setEnabled(True); self.btn_repopulate.setEnabled(True)
        self.lbl_status.setText(f"● ERROR: {err[:50]}"); self.lbl_status.setStyleSheet("color:#ef4444;")
